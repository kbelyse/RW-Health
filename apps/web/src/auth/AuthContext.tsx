...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    ...
    if (r.data?.user !== undefined) setUser(r.data.user);
    else setUser(null);
  }, []);
  ...
  const login = useCallback(async (email: string, password: string) => {
    ...
    if ("queued" in r && r.queued) return { ok: false as const, error: "You are offline" };
    if (!("data" in r) || !r.data?.user) return { ok: false as const, error: "Unexpected response" };
    setUser(r.data.user);
